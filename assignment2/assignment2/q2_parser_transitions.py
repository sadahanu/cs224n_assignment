class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition == "S":
            ...
            last_w = self.stack.pop()
            second_last_w = self.stack.pop()
            if transition == "LA":
                self.dependencies.append((last_w, second_last_w))
                self.stack.append(last_w)
            elif transition == "RA":
                self.dependencies.append((second_last_w, last_w))
                self.stack.append(second_last_w)
        ### END YOUR CODE
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while unfinished_parses:
        ...
        temp_unfinished_parses= []
        for partial_parse in unfinished_parses:
            if (len(partial_parse.buffer)==0) and (len(partial_parse.stack) == 1):
                continue
            else:
                temp_unfinished_parses.append(partial_parse)
        unfinished_parses = temp_unfinished_parses
